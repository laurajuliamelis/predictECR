...
ui <- navbarPage(
  ...
  tabPanel("The project",
           fluidPage(
             tags$p("Farmacogenètica Aplicada per Predir la Resposta al Tractament del Primer Episodi Psicòtic")
           )
  ),
  
  # 2. Tool and Details in dropdown
  navbarMenu("ECR Predictor",
             tabPanel("The tool",
                      fluidPage(
                        sidebarLayout(
                          sidebarPanel(
                            tags$h3("Patient Inputs", style = "font-weight: bold; color: #3b5773;",tooltip(
                              bsicons::bs_icon("info-circle-fill", class = "text-info"),
                              "PAS scores, education, and IQ are standardized using the PEPS cohort and projected onto a PCA axis to compute cognitive reserve. WCST values are normalized using PEPS control data.",
                              placement = "right"
                            )),
                            ...
                            numericInput("PAS_infancia", "PAS Score: childhood", value = 0, step = 0.1),
                            numericInput("PAS_adolescencia", "PAS Score: early adolescence", value = 0, step = 0.1),
                          ...
                          mainPanel(
                            tags$p("WARNING: Tool for research use only", style = "color: #db3d16; font-weight: bold; text-align:right;vertical-align: middle"),
                            tags$h2("Welcome to the ECR Predictor", style = "color: #3498db; font-weight: bold;"),
                            tags$p("This tool is created to easily estimate the probability of early clinical recovery (ECR) in patients with a first episode of psychosis. More details can be found in the publication: ",
             ...
             tabPanel("Details",
                      fluidPage(
                        tags$h3("PEPS Sample", style = "color: #2980b9; font-weight: bold;"),
                        ...
                        tags$p("The exclusion criteria for the patients were:"),
                        tags$ul(
                        ...
                        tags$h3("Model-building details", style = "color: #2980b9; font-weight: bold;"),
                        ...
                        tags$p("*Cognitive reserve was summarised  via Principal Component Analysis (PCA) on the standardised varianles estimated IQ, education (years), and premorbid adjustment scores (PAS, childhood plus early adolescence)."),
                        tags$p("**Executive function values were normalized to z-scores ussing the values from a control population in the PEPS cohort")
                      )
             )
  ),
  
  # 3. Contact & credits
  tabPanel("Contact, Credits and Funding",
           fluidPage(
             br(),
             tags$h3("About the FarmaPRED-PEP Project", style = "color: #2980b9; font-weight: bold;"),
             ...
             tags$p("This tool is intended solely for research use and should not replace clinical judgment.", style = "color: #db3d16; font-weight: bold; text-align:center;vertical-align: middle"),
             
             tags$h4("Contact us", style = "color: #2980b9; font-weight: bold;"),
...
server <- function(input, output, session) {
  observeEvent(input$predict, {
    ...
    newdata <- data.frame(
      ...
      PAS_infancia_adolescencia = (input$PAS_infancia + input$PAS_adolescencia)
    )
